...
/* harmony export */   getSignedUrlsForPhotoshop: () => (/* binding */ getSignedUrlsForPhotoshop)
/* harmony export */ });
/* unused harmony exports isAzureConfigured, isStorageConfigured */
/**
 ...
 */
async function getSignedUrlsForPhotoshop(baseBuffer, layerBuffer, keyPrefix) {
  if (!isAzureConfigured()) {
    throw new Error('Photoshop API placement requires Azure storage. Set AZURE_STORAGE_* in .env.');
  }

  const { BlobServiceClient, generateBlobSASQueryParameters, BlobSASPermissions, StorageSharedKeyCredential } = await Promise.resolve(/* import() */).then(__webpack_require__.bind(__webpack_require__, 2463));
